...
import { ChevronRight, IndianRupee, RotateCcw, Clock, CheckCircle2, XCircle } from 'lucide-react'
...
type Filter = 'all' | 'earned' | 'refunded' | 'pending'

function paymentFilter(booking: Booking, filter: Filter) {
  if (filter === 'all') return true
  if (filter === 'earned') return booking.payment?.status === 'SUCCESS'
...
function paymentStatusBadgeVariant(status: string | undefined): 'success' | 'error' | 'warning' | 'info' | 'default' {
...
export function InstructorPayments() {
  ...
  const stats = useMemo(() => {
    ...
    return { earned, refunded, pending }
  }, [myBookings])
  ...
  const tabs: { key: Filter; label: string; count: number }[] = [
    ...
    { key: 'earned', label: 'Earned', count: myBookings.filter((b) => b.payment?.status === 'SUCCESS').length },
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        ...
        <p className="text-stone-500 text-sm mt-1">Your session earnings and refund history.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white border border-stone-100 rounded-2xl px-5 py-4 shadow-sm">
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-xs text-stone-400 font-medium uppercase tracking-wide">Total Earned</p>
          </div>
          <p className="text-2xl font-bold text-stone-800">{formatPrice(stats.earned)}</p>
          <p className="text-xs text-stone-400 mt-0.5">
            {myBookings.filter((b) => b.payment?.status === 'SUCCESS').length} paid session{myBookings.filter((b) => b.payment?.status === 'SUCCESS').length !== 1 ? 's' : ''}
          </p>
        </div>
      ...
      {filtered.length > 0 ? (
        <div className="space-y-3">
          {filtered.map((booking) => {
            ...
            return (
              <Link
                key={booking.id}
                to={`/instructor/sessions/${booking.id}`}
        ...
        <div className="text-center py-16 border-2 border-dashed border-stone-200 rounded-2xl">
          <IndianRupee size={36} className="text-stone-300 mx-auto mb-3" />
