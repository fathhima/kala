...
const getOtpExpiryRemaining = () =>
  getRemainingSeconds(PENDING_SIGNUP_EXPIRES_AT_KEY);

const getResendCooldownRemaining = () =>
  getRemainingSeconds(PENDING_SIGNUP_RESEND_AVAILABLE_AT_KEY);
...
export function VerifyOtp() {
  ...
  const expiresInMinutes = Math.ceil(otpExpiresIn / 60);
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      setOtpExpiresIn(getOtpExpiryRemaining());
  ...
  const clearPendingSignupSession = () => {
    ...
    sessionStorage.removeItem(PENDING_SIGNUP_RESEND_AVAILABLE_AT_KEY);
  };
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      sessionStorage.setItem(
        PENDING_SIGNUP_EXPIRES_AT_KEY,
        String(Date.now() + response.expiresIn * 1000),
      );
      sessionStorage.setItem(
        PENDING_SIGNUP_RESEND_AVAILABLE_AT_KEY,
        String(Date.now() + response.resendAfter * 1000),
      );
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-stone-100 p-8">
          ...
          <p className="text-sm text-stone-500 mb-6">
            ...
            . It expires in {expiresInMinutes} minute
